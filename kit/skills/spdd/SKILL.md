...
- **You drive each step yourself, one phase at a time.** Two mechanics both work: (a) invoke
  the step via the Skill tool — commands are skills, and the model may call
  any skill that isn't `disable-model-invocation`; or (b) `Read commands/spdd-*.md`
...
- Keep gates light: summary + critique + one question. No walls of text. Honor `conventions.md`
...
4. **Generate** — procedure: `commands/spdd-generate.md`. Then run `pint` + `phpstan`.
   Report each file ↔ which Operation it implements.
5. **Test** — procedure: `commands/spdd-api-test.md`. Run `pest`. Report pass/fail honestly.
6. **Review** — procedure: `commands/spdd-code-review.md`. Present the `ALIGNED`/`DRIFT` verdict.
